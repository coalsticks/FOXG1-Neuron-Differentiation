...
"Showing similar distribution between medical and temporal conditions"
...
"The MA plot should be centered around M=0, the MD plot should be a straight line for all couple of samples"
...
# MD plot
MDPlot(raw_count, c(1, 13))
```

"This is just to check the expression level of one gene across samples"
...
"We filter to avoid non expressed genes or experimental or systematic errors"

```{r}

sample_11DIV <- sample_meta |> # Here we take the names for the samples at 11 (or 31) DIV, this is a list of vectors
...
raw_count_11 <- raw_count[, unlist(sample_11DIV)] # Here we subset the raw counts keeping only the samples with condition DIV=11 (or 31)
raw_count_31 <- raw_count[, unlist(sample_31DIV)]
#View(raw_count_11)
#View(raw_count_31)

#keep_11 <- rowSums(raw_count_11 < 10) <= (ncol(raw_count_11)/2)
filtered_raw_count_11 <- raw_count_11[rowSums(raw_count_11 < 20) <= (ncol(raw_count_11)/2), ] # Here we filter the genes, keeping only those with a value above threshold in at least half of the samples
#keep_31 <- rowSums(raw_count_31 < 10) <= (ncol(raw_count_31)/2)
filtered_raw_count_31 <- raw_count_31[rowSums(raw_count_31 < 20) <= (ncol(raw_count_31)/2), ]

common_rows <- intersect(rownames(filtered_raw_count_11), rownames(filtered_raw_count_31)) # Here we find only the genes that, after filtering in both conditions, have a significant expression level in both
...
raw_count <- raw_count[common_rows,] # We filter on the original raw counts matrix, keeping all samples but removing genes with too low of an expression level

data_filtered <- data[rownames(raw_count),] # We create a copy of the orginal dataset, using only the filtered genes
...
"It's here twice, probably this one is unnecessary"
...
data_filtered <- data_filtered[rowData(data_filtered)$ensid %in% rownames(gc), ] # We keep only the genes present in the online dataset
raw_count <- assay(data_filtered, "raw_counts") # We filter the raw counts variable too
row.names(raw_count) <- row.names(gc) # We use the new naming convention, to be sure every following step works
...
"Distribution of the median pretty constant, i guess this is fine"
...
corrplot(m, method = "color", order = "hclust", tl.cex = 0.2,
         main = "Correlation - TMM", col.lim = c(0.5, 1)) # This is the true correlation matrix, it shows high correlation across all samples but it is not clear if there are major differences
corrplot(m^3, method = "color", order = "hclust", tl.cex = 0.2,
         main = "Enhanced Correlation", col.lim = c(0.5, 1)) # This is not a real correlation matrix, since we raise every number to the third power, however it allows us to more easily distinguish small value differences
#colnames(tmm) <- tmm_colnames
```

### PCA After Normalization

"This shows clustering with the 11-31 DIV condition and no clustering with ASD-nonASD condition"
...
dge <- DGEList(raw_count) # use normalized raw count -rle same results
